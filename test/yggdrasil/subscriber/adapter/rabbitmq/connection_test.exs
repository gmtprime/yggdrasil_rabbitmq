defmodule Yggdrasil.Subscriber.Adapter.RabbitMQ.ConnectionTest do
  ...
  test "start/stop" do
    assert {:ok, pid} = Conn.start_link(ConnectionNamespace0)
    assert :ok = Conn.stop(pid)
  end

  test "get_connection/1" do
    assert {:ok, pid} = Conn.start_link(ConnectionNamespace1)
  ...
  test "calculate_backoff/1" do
    ...
    retries = 3
    state = %Conn{namespace: ConnectionNamespace2, retries: retries}
